        ...
        <section class="import-card">
            <div class="import-card-head">
                ...
                <div>
                    ...
                    <p>Add each new product as a new row. Only Product Name is required; blank tare becomes 0, blank unit becomes kg, and the minimum/maximum weight and customer barcode columns are optional.</p>
                </div>
            </div>
                    ...
                    @elseif(($productPreview['records'] ?? []) !== [])
                        <div class="table-wrap">
                            <table class="data-table">
                                <thead><tr><th>Product</th><th>Code</th><th>Tare</th><th>Weight Range</th><th>Unit</th><th>Customer Barcode</th></tr></thead>
                                <tbody>
                                    @foreach(array_slice($productPreview['records'], 0, 20) as $row)
                                        <tr>
                                            ...
                                            <td>{{ $row['minimum_weight'] !== null ? number_format($row['minimum_weight'], 3).'–'.number_format($row['maximum_weight'], 3).' kg' : '-' }}</td>
                                            <td>{{ $row['unit'] }}</td>
